...
# 📚 API Docs: http://localhost:2010/docs
# ✅ Health Check: curl http://localhost:2010/health
...
# 2. Start API server
python flow_api.py
# API runs on http://localhost:2010
...
│  │   (Web)      │  │  (API)       │  │  (AI)      │ │
│  │              │  │              │  │            │ │
│  │ Port: 2009   │  │ Port: 2010   │  │Port: 11434 │ │
│  │              │  │              │  │            │ │
...
curl http://localhost:2010/api/cache/stats

# Clear cache for specific symbol
curl -X DELETE http://localhost:2010/api/cache/AAPL

# Remove all expired entries
curl -X DELETE http://localhost:2010/api/cache/expired

# Flush entire cache
curl -X POST http://localhost:2010/api/cache-flush
```
...
# API Docs: http://192.168.1.248:2010/docs
```
...
Edit `web/config.js`:
```javascript
const API_BASE_URL = 'http://192.168.1.248:2010'; // Change to your API server
```
...
```bash
# Get fundamentals
curl http://localhost:2010/api/fundamentals/AAPL

# Get AI fundamental analysis
curl http://localhost:2010/api/ai/fundamental-analysis/AAPL

# Get news sentiment
curl http://localhost:2010/api/ai/news-sentiment/CHTR

# Interactive API documentation
open http://localhost:2010/docs
```
...
5. Verify: `curl http://localhost:2010/health`
...
docker compose up -d -p 8001:2009
```
