...
class TestWebUIDataEndpoints(unittest.TestCase):
    ...
    def test_fundamentals_endpoint_returns_json_serializable(
        self, mock_get_fundamentals
    ):
    ...
    def test_price_history_endpoint_format_for_candlestick_chart(
    ...
    @patch("flow.get_fundamentals")
    def test_fundamentals_supports_human_readable_market_cap(
        self, mock_get_fundamentals
    ):
    ...
    @patch("flow.ensure_ollama")
    def test_news_sentiment_ai_analysis_endpoint_on_results_page(
        self, mock_ensure_ollama
    ):
...
class TestWebUIResultsPageLayout(unittest.TestCase):
    ...
    @patch("flow.ensure_ollama")
    def test_news_sentiment_ai_analysis_is_only_ai_card_displayed(
        self, mock_ensure_ollama
    ):
        """Test News & Sentiment AI Analysis is the only AI analysis card on results page."""
        with patch("flow.get_ai_news_sentiment") as mock_sentiment:
            mock_sentiment.return_value = (
                "Recent news sentiment is positive with strong investor confidence."
            )
    ...
    def test_fundamentals_supports_dividend_indicator(self, mock_get_fundamentals):
        ...
        if "dividendYield" in data:
            ...
            self.assertLessEqual(
                data["dividendYield"], 1
            )  # Should be decimal (not percentage)
...
class TestCORSHeadersForWebUI(unittest.TestCase):
    ...
    def test_cors_headers_allow_cross_origin(self, mock_get_fundamentals):
        ...
        response = self.client.get(
            "/api/fundamentals/AAPL", headers={"Origin": "http://localhost:8081"}
        )
