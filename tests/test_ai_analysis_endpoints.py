...
class TestAIAnalysisEndpoints(unittest.TestCase):
    ...
    def test_fundamental_analysis_endpoint(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_fundamental_analysis")
    def test_fundamental_analysis_endpoint_error(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    def test_balance_sheet_analysis_endpoint(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_balance_sheet_analysis")
    def test_balance_sheet_analysis_endpoint_error(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    def test_income_stmt_analysis_endpoint(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
        """Test income statement analysis endpoint."""
        mock_analysis.return_value = (
            "Revenue growth is impressive with expanding margins."
        )
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_quarterly_income_stm_analysis")
    def test_income_stmt_analysis_endpoint_error(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_technical_analysis")
    def test_technical_analysis_endpoint_error(
        self, mock_analysis, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    def test_action_recommendation_endpoint(
        self, mock_recommendation, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
        """Test action recommendation endpoint."""
        mock_recommendation.return_value = (
            "BUY - Strong fundamentals with positive momentum."
        )
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_action_recommendation")
    def test_action_recommendation_endpoint_error(
        self, mock_recommendation, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    def test_action_recommendation_sentence_endpoint(
        self, mock_rec_sentence, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_action_recommendation_sentence")
    def test_action_recommendation_sentence_endpoint_error(
        self, mock_rec_sentence, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    def test_action_recommendation_word_endpoint(
        self, mock_rec_word, mock_ensure_ollama
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_action_recommendation_single_word")
    def test_action_recommendation_word_endpoint_error(
        self, mock_rec_word, mock_ensure_ollama
    ):
...
class TestGetNewsSentiment(unittest.TestCase):
    ...
    def setUp(self):
        """Isolate sentiment tests from shared cache state."""
        self.cache_get_patcher = patch("db.get_cached_data", return_value=None)
        self.cache_set_patcher = patch("db.set_cached_data")
        self.mock_get_cached_data = self.cache_get_patcher.start()
        self.mock_set_cached_data = self.cache_set_patcher.start()
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_with_valid_news(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        """Test news sentiment analysis with valid news data."""
        # Mock news data
        mock_get_news.return_value = [
            {"content": {"summary": "Company reports strong quarterly earnings"}},
            {"content": {"summary": "Stock price reaches 52-week high"}},
        ]

        # Mock Ollama response
        mock_response = MagicMock()
        mock_response.message.content = (
            "The sentiment is positive with strong earnings and price appreciation."
        )
    ...
    def test_news_sentiment_no_summaries(self, mock_get_news):
        ...
        mock_get_news.return_value = [{"content": {}}, {"content": {}}]
        result = flow.get_ai_news_sentiment("XYZ")
        self.assertEqual(
            result,
            "No news summaries available for analysis. Check back later for market news.",
        )
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_uses_top_5_articles(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        """Test that only top 5 articles are used for analysis."""
        # Create 10 news items
        mock_get_news.return_value = [
            {"content": {"summary": f"News item {i}"}} for i in range(10)
        ]
        ...
        flow.get_ai_news_sentiment("AAPL")
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_positive_response(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_negative_response(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_mixed_response(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        ...
        mock_response = MagicMock()
        mock_response.message.content = (
            "The sentiment is mixed with both positive and negative factors."
        )
        mock_chat.return_value = mock_response
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_uses_top_five_articles(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        """Test that only top 5 articles are used for analysis."""
        # Create 10 mock articles
        articles = [{"content": {"summary": f"Article {i} summary"}} for i in range(10)]
        ...
        flow.get_ai_news_sentiment("TEST")
        ...
        # Current implementation uses the top 3 summaries for faster responses.
        self.assertIn("Article 0 summary", content)
        self.assertIn("Article 1 summary", content)
        self.assertIn("Article 2 summary", content)
        self.assertNotIn("Article 3 summary", content)

    @patch("flow.get_news")
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_error_handling(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        """Test error handling in news sentiment analysis."""
        mock_get_news.return_value = [{"content": {"summary": "Some news"}}]
    ...
    @patch("flow.ensure_ollama")
    @patch("flow._CHAT")
    def test_news_sentiment_extracts_content_correctly(
        self, mock_chat, mock_ensure_ollama, mock_get_news
    ):
        ...
        mock_get_news.return_value = [
            ...
            {"content": {"summary": "Analyst upgrades target price"}},
        ]
...
class TestNewsSentimentAPI(unittest.TestCase):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_endpoint_success(
        self, mock_get_sentiment, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_endpoint_none_result(
        self, mock_get_sentiment, mock_ensure_ollama
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_endpoint_various_symbols(
        self, mock_get_sentiment, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_response_format(
        self, mock_get_sentiment, mock_ensure_ollama
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_case_insensitive(
        self, mock_get_sentiment, mock_ensure_ollama
    ):
    ...
    @patch("flow.ensure_ollama")
    @patch("flow.get_ai_news_sentiment")
    def test_news_sentiment_with_special_characters_in_symbol(
        self, mock_get_sentiment, mock_ensure_ollama, mock_set_cache, mock_get_cache
    ):
